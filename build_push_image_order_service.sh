...
docker tag order-service:latest ardaiyansyah/order-service:latest
...
docker push ardaiyansyah/order-service:latest
 
echo "Proses build dan push selesai pada Dockerhub!"
 
# mengubah nama image agar sesuai dengan format GitHub Packages
echo "Mengubah nama image..."
docker tag order-service:latest ghcr.io/macreai/order-service:latest
 
# login ke Github Packages
echo $CR_PAT | docker login ghcr.io -u macreai --password-stdin
 
# mengunggah image ke GitHub Packages
docker push ghcr.io/macreai/order-service:latest
